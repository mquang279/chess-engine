...
void ChessGUI::initialize()
{
    ...
    homeButtonText.setPosition(
        ...
        homeButton.getPosition().y + (buttonHeight - homeTextBounds.height) / 2 - homeTextBounds.top + 4);
    ...
    backButtonText.setPosition(
        ...
        backButton.getPosition().y + (buttonHeight - backTextBounds.height) / 2 - backTextBounds.top + 4);
}
        ...
        while (window.pollEvent(event))
        {
            ...
            else if (event.type == sf::Event::MouseButtonPressed)
            {
                if (event.mouseButton.button == sf::Mouse::Left)
                {
                    ...
                    if (inSelectionMode)
                    {
                        handleSelectionScreenMousePress(clickPosition);
                    }
                    else
                    {
                        // Check if any button was clicked first
                        if (clickPosition.y >= boardHeight - statusBar.getSize().y - homeButton.getSize().y - 20)
                        {
                            handleButtonClick(clickPosition);
                        }
                        else
                        {
                            handleMousePressed(clickPosition);
                        }
                    }
                }
            }
            ...
            else if (event.type == sf::Event::MouseMoved)
            {
                ...
                if (!inSelectionMode)
                {
                    ...
                    isHomeButtonHovered =
                        mousePos.x >= homeButton.getPosition().x &&
                        mousePos.x <= homeButton.getPosition().x + homeButton.getSize().x &&
                        mousePos.y >= homeButton.getPosition().y &&
                        mousePos.y <= homeButton.getPosition().y + homeButton.getSize().y;

                    isBackButtonHovered =
                        mousePos.x >= backButton.getPosition().x &&
                        mousePos.x <= backButton.getPosition().x + backButton.getSize().x &&
                        mousePos.y >= backButton.getPosition().y &&
                        mousePos.y <= backButton.getPosition().y + backButton.getSize().y;
...
void ChessGUI::resetToSelectionScreen()
{
...
void ChessGUI::undoLastMove()
{
    // Check if we have moves to undo
    chess::Movelist moves;
    chess::movegen::legalmoves(moves, board);

    // If there are legal moves, it means we're not at the beginning position
    if (board.fullMoveNumber() > 1 || board.sideToMove() == chess::Color::BLACK)
    {
        // Create a dummy move to undo
        chess::Move moveToUndo = chess::Move::NULL_MOVE;

        // First undo - undo the opponent's last move
        board.unmakeMove(moveToUndo);

        // Second undo - undo your own last move
        // Check if we're not already at the beginning position
        if (board.fullMoveNumber() > 1 || board.sideToMove() == chess::Color::BLACK)
        {
            board.unmakeMove(moveToUndo);
        }
...
void ChessGUI::handleButtonClick(sf::Vector2i position)
{
    // Check if the home button was clicked
    if (position.x >= homeButton.getPosition().x &&
        position.x <= homeButton.getPosition().x + homeButton.getSize().x &&
        position.y >= homeButton.getPosition().y &&
        position.y <= homeButton.getPosition().y + homeButton.getSize().y)
    {
        // Reset the game and return to selection screen
        resetToSelectionScreen();
    }
    // Check if the back button was clicked
    else if (position.x >= backButton.getPosition().x &&
             position.x <= backButton.getPosition().x + backButton.getSize().x &&
             position.y >= backButton.getPosition().y &&
             position.y <= backButton.getPosition().y + backButton.getSize().y)
    {
        // Undo the last move
        undoLastMove();
    }
